...
We created three dictionaries: UNESCO dictionary, business dictionary and book dictionary.

These are ultimately combined into the master dictionary file `dictionaries_dataset_all_words` and several subset/subsidiary files in the `./output/created_datasets` folder.

### UNESCO (Open Research) dictionary <br>
The first dictionary (UNESCO (Open Research) dictionary) was created by removing stop words and choosing 100 most frequent words in the UNESCO Recommendations on Open Science (https://unesdoc.unesco.org/ark:/48223/pf0000379949.locale=en). 

### Business dictionary <br>
The second one (Business dictionary) was created by importing business glossaries from four websites (see below). 100 words that occurred the most frequent were used to create this dictionary. 
Websites:
...
Control dictionary <br>
The third one (Control dictionary) was created by removing stop words and identifying 100 most common words in the ecology book "Our Vanishing Wildlife" downloaded from the online library Project Gutenberg.
...
# Setup for working with the data
...
# Preparing the dictionary dataset
...
# Basic statistics of dictionary data
...
# Saving the total data (freq_dict_total_all)
...
# Creating dictionaries (all words, 100 unique words)
...
# Saving freq_dict_total_100 data
