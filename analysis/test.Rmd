...
The key stakeholders relevant in ecological and evolutionary research landscape (for and non-for profit publishers, OA and other journals, learned societies, repositories, funders and stakeholders actively promoting OR movement) will be identified in advance. Aims, missions, strategic plans and other documents describing their values must be publicly available on their official websites. Our objective is to analyse all documents together of each type of stakeholder to identify their topics of focus and messages they want to communicate. I will analyse the documents which are publicly accessible and I will not contact anyone associated with the stakeholder to request more information. 

For each stakeholder, I will identify eligible website or document. For each document I will manually copy its text, copy the source link to the first line and save as a text document (.txt).

I have started with up to 5 representatives of each type and in the future I will increase my sample size. 

Wordclouds will be generate for the each type of the stakeholders to identify the most relevant words used in their documents. 

Topic modelling uncover latent topics within a corpus of documents. In my analysis I will conduct the Structural Topic Model (STM) and Latent Dirichlet Allocation (LDA).
...
# Git settings
To check if I already have an SSH key-pair by running:
...
```{r}
# create new columns: Name, Organisation
...
# corpus_df$stakeholder[corpus_df$stakeholder%in% c("UNESCO Recommendation on Open Science" )] <- "recommendation"

unique(corpus_df$stakeholder)

# removing the first string from the each document that is a source link
# corpus_df$txt <- sub(".*?\r","",corpus_df$txt)
```

``` {r}
# saving the dataframe
# write_excel_csv(corpus_df, "corpuses_preprocessing.csv")

```

``` {r}
# Saving a file with the document info
head(corpus_df)
info <- corpus_df %>% 
  select(txt, filename, organisation = name, stakeholder)

info$stakeholder_more <- info$stakeholder

info$stakeholder[info$stakeholder%in% c("journal_OA", "journal_nonOA" )] <- "journal"
info$stakeholder[info$stakeholder%in% c("publisher_Profit", "publisher_nonProfit" )] <- "publisher"

head(info)

library(stringr)
info$website <- word(info$txt, 1)

# write_excel_csv(info, "documents_info.csv")

```

Maier et al. 2018 "Applying LDA Topic Modelling in Communication Research: Toward a Valid and Reliable Methodology" 
(a) preprocessing: 
- cleaning the text from http:// and https:// links
- tokenisation
- capital letters to lowercase
- punctuation and special characters deleted
- removal of stop-words: check the lexicons in stop_words, remove words like: numbering (ii, iii, iv, v), name of document type (aim, aims, mission...), name of the stakeholders (erc, nerc, wellcome)
...
Recommended to use lemmatization instead of stemming
- relative prunning
relative pruning is recommended to strip very rare and extremely frequent word occurrences from the observed data:, relative pruning reduces the size of the corpus vocabulary, which will enhance the algorithm’s performance remarkably (Denny & Spirling, 2017) and will stabilize LDA’s stochastic inference. In our empirical study, relative pruning was applied, removing all terms that occurred in more than 99% or less than .5% of all documents (Denny & Spirling, 2017; Grimmer, 2010; Grimmer & Stewart, 2013).
...
data_tidy_sentences <- corpus_df %>%  
  # unnest_tokens(sentence, txt_copy, token = "regex", pattern = "(?<=\\.|\\?)\\s(?=[A-Z])")
  unnest_tokens(sentence, txt_copy, token = "sentences")

head(data_tidy_sentences,5)
...
data_words$stakeholder[data_words$stakeholder%in% c("journal_OA", "journal_nonOA" )] <- "journal"
data_words$stakeholder[data_words$stakeholder%in% c("publisher_Profit", "publisher_nonProfit" )] <- "publisher"
unique(data_words$stakeholder)
```

``` {r}
unique(data_words$stakeholder)
# This data will be used to create wordclouds in other script and Ternary_plot_final
# write_csv(data_words, "corpuses_data_1.csv")
head(data_words,5)
# 
# check <- data_words %>% 
#   select(-sentence_doc, -txt,  )
